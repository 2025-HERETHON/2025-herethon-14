...
function renderTable() {
  ...
  tbody.innerHTML =
    pageData.length === 0
      ? `<tr class="storage-row"><td colspan="3" style="color:#bbb;text-align:center;">작성된 글이 없습니다.</td></tr>`
      : pageData
          .map(
            (row, i) => `
  ...
  document.querySelectorAll(".storage-row").forEach((tr) => {
    tr.addEventListener("click", () => {
      window.location.href = "timelog_post.html";
    });
  });
}
