...
function renderFilesLinks() {
  ...
  uploadedFiles.forEach((file, idx) => {
    ...
    div.innerHTML = `<img src="icon-file.svg" style="width:16px;"> <span>${file.name}</span> <span class="remove-attach" title="삭제">×</span>`;
  ...
  uploadedLinks.forEach((link, idx) => {
    ...
    div.innerHTML = `<img src="icon-link.svg" style="width:15px;"> <a href="${link}" target="_blank" style="color:#06c; text-decoration:underline;">${link}</a> <span class="remove-attach" title="삭제">×</span>`;
    div.querySelector(".remove-attach").onclick = () => {
