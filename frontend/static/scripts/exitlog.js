const cards = [
    {
        username: '김** 님',
        title: '여러분의 잘못이 아니에요',
        content: '처음엔 그냥 제가 예민한 줄 알았어요 ㅠㅠ 하루 종일 연락 안 받았다고 혼나는 것도, 친구랑 만났다고 화내는 것도,다 나를 너무 사랑해서 그런 줄 알았거든요.',
        date: '2025.06.02',
        isScrapped: false
    },
    {
        username: '이** 님',
        title: '이제는 웃고 있어요',
        content: '안녕하세요 안녕하세요 이렇게저렇게해서 고소했고 접근금지 신청했어요 여러분에게 도움을 주고자 이 글을 써요 꼭 이겨내서 같이 행복한 삶을 살...',
        date: '2025.06.01',
        isScrapped: false
    },
    {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    cards.forEach(card => {
        card.isScrapped = saved.some(savedCard =>
            ...
            <div class="escape-card">
                <div class="card-header">
                    ...
                    <button class="scrap-icon" data-index="${index}">
                        <img src="../assets/${card.isScrapped ? 'scrab.png' : 'nonscrab.png'}" />
                    </button>
                </div>
                <div class="card-body">
...
document.addEventListener('click', function (e) {
    ...
    if (button) {
        ...
        img.src = `../assets/${cards[index].isScrapped ? 'scrab.png' : 'nonscrab.png'}`;
...
function selectSort(option) {
    ...
    document.querySelector('.sort-trigger').innerHTML = `${option} <span class="arrow">▼</span>`;
}

function sortPosts(order) {
    if (order === 'old') {
        cards.sort((a, b) => new Date(a.date.replace(/\./g, '-')) - new Date(b.date.replace(/\./g, '-')));
    } else {
        cards.sort((a, b) => new Date(b.date.replace(/\./g, '-')) - new Date(a.date.replace(/\./g, '-')));
    }
    renderCards(cards);
}
...
document.addEventListener('click', function (e) {
    ...
    if (button) {
        ...
        const exists = saved.find(card => card.title === title && card.username === username && card.date === date);

        if (exists) {
            // 삭제
            saved = saved.filter(card => !(card.title === title && card.username === username && card.date === date));
